import express from "express";

import ConsecutiveService from "../services/consecutive.service";
import {
  getIdConsecutiveSchema,
  getIdQueryConsecutiveSchema,
  postConsecutiveSchema,
  patchConsecutiveSchema,
} from "../schemas/consecutive.schema";
import validatorHandler from "../middlewares/validator.handler";

const router = express.Router();
const service = new ConsecutiveService();

router.get(
  "/",
  validatorHandler(getIdQueryConsecutiveSchema, `query`),
  async (req, res, next) => {
    try {
      let result = [];
      if (req.query._id) {
        result = await service.findOne(req.query._id);
      } else {
        result = await service.find({});
      }
      res.status(200).json(result);
    } catch (error) {
      next(error);
    }
  }
);

router.post(
  "/",
  validatorHandler(postConsecutiveSchema, `body`),
  async (req, res, next) => {
    try {
      const result = await service.create(req.body);
      res.status(200).json(result);
      next();
    } catch (error) {
      next(error);
    }
  }
);

router.patch(
  "/",
  validatorHandler(getIdConsecutiveSchema, `query`),
  validatorHandler(patchConsecutiveSchema, `body`),
  async (req, res, next) => {
    try {
      const result = await service.update(req.query._id, req.body);
      res.status(200).json(result);
      next();
    } catch (error) {
      next(error);
    }
  }
);

router.delete(
  "/",
  validatorHandler(getIdConsecutiveSchema, `query`),
  async (req, res, next) => {
    try {
      const result = await service.delete(req.query._id);
      res.status(200).json(result);
      next();
    } catch (error) {
      next(error);
    }
  }
);

export default router;
